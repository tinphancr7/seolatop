...
const RankItem = ({dataOrder, orderActive, name, item}: any) => {
	return (
		<div className="relative group">
				...
				className={`w-full h-[208px] lg:h-[476px] relative flex-shrink-0 carousel-item  skew-x-[-10deg]  ${
					name === "seo"
						...
						: `${
						  ...
						  }   after:absolute after:inset-0 after:bg-custom-gradient after:opacity-0 after:transition after:duration-700 after:ease-in-out  hover:after:opacity-100`
				}   `}
			...
			>
				...
				<div className=" absolute top-[50%] -translate-y-[50%] left-[50%] -translate-x-[50%] z-[9999] flex flex-col items-center justify-center skew-x-[10deg] ">
					<div
						className={` flex-shrink-0 relative group-hover:animate-custom-fade-in  ${rankLogo[name]?.width} ${rankLogo[name]?.height} `}
					>
						<Image
							src={`/images/${
			...
			{name === "diamond" && item?.bio && (
				<div className="bg-custom-gradient w-full h-fit rounded-lg absolute -left-5 top-[350px] p-5 right-0  z-10 hidden group-hover:block">
					{item?.bio}
				</div>
			)}
		</div>
